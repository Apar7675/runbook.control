...
export type RBDevice = { id: string; shop_id: string; name: string; device_key: string; status: string; created_at: string };
export type RBUpdatePolicy = { id: string; shop_id: string; channel: string; min_version: string | null; pinned_version: string | null; created_at: string };
...
export async function rbGetShop(shopId: string): Promise<RBShop> {
  const supabase = await supabaseServer();
  const { data, error } = await supabase
    .from("rb_shops")
    .select("*")
