...
export async function GET(req: Request) {
  try {
    ...
    rateLimitOrThrow({ key: `people:list:${ip}`, limit: 120, windowMs: 60_000 });
    ...
    if (trustedDeviceRows.error) {
      return NextResponse.json({ ok: false, error: trustedDeviceRows.error.message }, { status: 500 });
    }
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        trusted_device_count: users.reduce((sum, row) => sum + row.trusted_device_count, 0),
      },
      users,
    });
  } catch (e: any) {
