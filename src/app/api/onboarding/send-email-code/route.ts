...
import { sendOnboardingEmailCode } from "@/lib/onboarding/messaging";
...
export async function POST(req: Request) {
  try {
    ...
    rateLimitOrThrow({ key: `onboarding:email:ip:${ip}`, limit: 8, windowMs: 10 * 60_000 });
    rateLimitOrThrow({ key: `onboarding:email:addr:${email}`, limit: 5, windowMs: 10 * 60_000 });
    ...
    const delivery = await sendOnboardingEmailCode({ email, code, fullName: full_name });

    return NextResponse.json({
      expires_at: issue.expires_at,
      resend_available_in: issue.resend_available_in,
      message: "Email code sent. Use it to confirm this account before continuing.",
      ...delivery,
    });
  } catch (e: any) {
