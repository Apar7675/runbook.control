  ...
  return (
    <div className="rb-page">
      <PageHeader
        ...
        actions={<>{returnTo ? <ActionLink href={returnTo}>Return to Setup</ActionLink> : null}<ActionLink href="/devices" tone="primary">Open Device Center</ActionLink><ActionLink href={`/shops/${snapshot.id}/audit`}>Review Activity</ActionLink></>}
      />

      <SectionBlock title="Health Summary" description="A novice admin should not have to parse raw device records first." tone={health === "Healthy" ? "healthy" : health === "Warning" ? "warning" : "critical"}>
        <div className="rb-chipRow"><StatusBadge label={health} tone={toneFromStatus(health)} /></div>
      ...
      <div className="rb-autoGrid">
        <StatCallout label="Desktop" value={snapshot.counts.desktops_active} detail={`Active desktops of ${snapshot.counts.desktops_total} total.`} />
        <StatCallout label="Workstation" value={snapshot.counts.workstations_active} detail={`Active workstations of ${snapshot.counts.workstations_total} total.`} />
      </div>
