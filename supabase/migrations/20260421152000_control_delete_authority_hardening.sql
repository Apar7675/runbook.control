  ...
  if v_employee.auth_user_id is not null then
    ...
    if v_membership_role in ('owner', 'admin') then
      ...
      if v_admin_count <= 1 then
        raise exception 'cannot remove the last owner/admin-backed employee from the shop';
      end if;
    end if;
  end if;
...
revoke all on function public.rb_delete_shop_authoritative(uuid, text, uuid) from public;
revoke all on function public.rb_delete_shop_authoritative(uuid, text, uuid) from anon;
revoke all on function public.rb_delete_shop_authoritative(uuid, text, uuid) from authenticated;
grant execute on function public.rb_delete_shop_authoritative(uuid, text, uuid) to service_role;
