...
BEGIN
  ...
  IF NOT EXISTS (
    ...
    ALTER TABLE public.employees ADD COLUMN source_local_employee_id integer;
  END IF;
END
$$;
...
SET
  ...
  status = CASE WHEN is_active THEN 'Active' ELSE COALESCE(NULLIF(status, ''), 'Inactive') END
WHERE
  full_name = ''
  OR preferred_name = ''
  OR status = '';
