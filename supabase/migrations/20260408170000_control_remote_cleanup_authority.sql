...
create table if not exists public.rb_remote_cleanup_commands (
  id uuid primary key default gen_random_uuid(),
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  issued_at timestamptz not null default now(),
  processed_at timestamptz,
  state text not null default 'pending' check (state in ('pending', 'acknowledged', 'completed', 'canceled')),
  target_app text not null check (target_app in ('desktop', 'workstation', 'mobile')),
  action text not null check (action in ('purge_shop_data', 'revoke_shop_access', 'clear_local_trust', 'reset_pairing')),
...
create index if not exists rb_remote_cleanup_commands_state_idx
  on public.rb_remote_cleanup_commands (state, issued_at desc);

create index if not exists rb_remote_cleanup_commands_auth_shop_idx
  on public.rb_remote_cleanup_commands (auth_user_id, shop_id, state, target_app);

create index if not exists rb_remote_cleanup_commands_device_idx
  on public.rb_remote_cleanup_commands (device_id, state, target_app);

create index if not exists rb_remote_cleanup_commands_shop_idx
  on public.rb_remote_cleanup_commands (shop_id, state, target_app);

create or replace function public.rb_issue_remote_cleanup_command(
  p_target_app text,
  p_action text,
  p_reason text,
...
declare
  v_id uuid;
begin
  insert into public.rb_remote_cleanup_commands (
    target_app,
    action,
    reason,
    shop_id,
